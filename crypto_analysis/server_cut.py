...
def get_indicators(symbol, interval):
    ...
    # Обработка первого файла для извлечения данных индикаторов
    for file in files:
    ...
    # Ограничение вывода до 100 строк
    if len(data) > 100:
        data = data[:100]

    # Формируем ответ с данными для анализа, без секции волатильности
    response = OrderedDict()
    response["Data for Analysis"] = data
